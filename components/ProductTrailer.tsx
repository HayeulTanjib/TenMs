...
const ProductTrailer = ({ data }: { data: MediaData }) => {
  ...
  return (
    <section className='w-96 h-[650px] p-4 border border-gray-300 rounded-md'>
      <div className='relative'>
        {!isVideoOpen && (
          <>
            <img height='900' width='400' className='brightness-[.6]' loading='lazy' src={data.media[0].thumbnail_url} alt='' />
        ...
        {isVideoOpen && (
          <iframe
            width='382'
            height='205'
            src={`https://www.youtube.com/embed/${data?.media[0]?.resource_value}?si=ZBsvcrurI2mA87ub`}
